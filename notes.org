...
| kwee1-win10  | win10  | d:/cprojects.2025/github/play |
| rooibos      | ubuntu | ~/projects/github/play        |
...
*** iching
**** Idea
This would be a series of simple tools
  | 1 | showfiles sf | given a list of hexagram numbers, open the appropriate .jpg in a standard location |   |
  | 2 | showlist sl  | given a list of hexagrams, create a .html that displays the list                 |   |
  | 2 |              | straightline and circular                                                        |   |
  | 3 | showops so   | given a list of hexagrams, derive its derivatives                                |   |
  | 3 |              | derivatives are: inverse, converse, nuclear                                      |   |

Definition of hexagram list: currently start with a number of may enhance into
h.m1,m2 etc as simple format for hexagram and moving lines.
Q) Develop with python or c#
   Later, might do both but for quick and simple tasks, Python is faster and there are more libraries
   But what about type checking?
**** Naming convention?
yj-show
ic-show
...
Q? Which machine is this? I think given the c:\programs, it is kwee1-windows
...
**** surgically refactor resume so that resume1 can open any configuration file
Was going to use kh-lotus organization but realized that nealford/emacs contains too much of other stuff.
So just keep in play/elisp/revive

This thread was to study in detail the original revive code.
revive0.el
revive1-outshine.el

I now have a working code that I surgically added resume1 to do what I need. It is simple and works
very well.

Q) Where should the distribution code be kept? revive.el

How about creating a folder called revive-distribution
  check in revive.el
  then the updated revive.el

**** DONE Use revive-dist to copy new revive.el to all machines
CLOSED: [2023-04-13 Thu 20:04]
- State "DONE"       from              [2023-04-13 Thu 20:04]
1. [X] kwee1 // cp revive.el c:/programs/emacs-28.2/emacs-28.2/share/emacs/site-lisp/revive.el
2. [X] kwee1-ubuntu // sudo cp to /usr/share/emacs/site-lisp
3. [X] air // similar to kwee1
4. [X] rooibos // sudo cp to /usr/local/share/emacs/site-lisp
...
*** rooibos: vnc viewer window takes up vertical space, causes win max to truncate
